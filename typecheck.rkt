...
(define typecheck-R2
  (λ (env expr)
    (match expr
      ...
      [`(if ,c ,t ,f)
       (match (typecheck-R2 env c)
         [`Boolean
          (let ([t-ty (typecheck-R2 env t)]
                [f-ty (typecheck-R2 env f)])
            (if (eqv? t-ty f-ty)
                t-ty
      ...
      [`(eq? ,e1 ,e2)
       ...
       (let ([e1-ty (typecheck-R2 env e1)]
             [e2-ty (typecheck-R2 env e2)])
         (if (eqv? e1-ty e2-ty)
             (match e1-ty
      ...
      [`(program ,body)
       (begin
         (typecheck-R2 `() body) ; Make sure everything is well typed...
